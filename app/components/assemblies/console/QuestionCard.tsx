...
export function QuestionCard({ question, tenantId, assemblyId, properties, onStatusChange }: QuestionCardProps) {
    ...
    return (
        <>
                    ...
                    <div className="absolute w-full h-full backface-hidden rotate-y-180 bg-slate-800 border border-white/10 rounded-xl p-6 flex flex-col shadow-xl">
                        ...
                        <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-2">
                            ...
                            <button
                                ...
                                className="w-full py-2 bg-slate-700 hover:bg-slate-600 text-xs text-indigo-300 rounded border border-slate-600 transition-colors"
                            >
            ...
            <InlineModal isOpen={showDetails} onClose={() => setShowDetails(false)} title="Detalle de Votación">
                <div className="space-y-4">
                    <div className="flex justify-between items-center bg-slate-800 p-3 rounded-lg border border-white/5">
                        <div className="text-sm font-medium text-white">Progreso de Votación</div>
                        <div className="text-xs text-slate-400">
                            {rawVotes.length} de {properties.length} propiedades
                        </div>
                    </div>

                    <div className="max-h-[400px] overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                        {properties.map(p => {
                            const vote = rawVotes.find(v => v.propertyId === p.id);
                            const hasVoted = !!vote;

                            return (
                                <div key={p.id} className="flex justify-between items-center p-2 rounded bg-slate-800/50 border border-white/5">
                                    <div className="overflow-hidden">
                                        <div className="text-sm text-white font-medium truncate">{p.name}</div>
                                        <div className="text-xs text-slate-400 truncate">{p.ownerName || 'Sin propietario'}</div>
                                    </div>
                                    <div>
                                        {hasVoted ? (
                                            <span className="px-2 py-1 rounded-full bg-green-900/30 text-green-400 border border-green-500/30 text-xs font-medium">
                                                Votó
                                            </span>
                                        ) : (
                                            <span className="px-2 py-1 rounded-full bg-red-900/20 text-red-400 border border-red-500/30 text-xs font-medium">
                                                Pendiente
                                            </span>
                                        )}
                                    </div>
